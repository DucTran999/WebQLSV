...
foreach ($studentList as $std) 
{
	...
	echo '<tr>
			...
			<td><button class="btn btn-info" onclick=\'window.open("xemm.php?id='.$std['id'].'","_self")\'>Đăng kí</button></td>
		</tr>';
	}
}
...
foreach ($studentList as $std) 
{
	...
	echo '<tr>
			...
			<td>'.$std['ma'].'</td>
			<td>'.$std['tin'].'</td>
			<td><button class="btn btn-danger" onclick="deleteDK('.$std['id'].')"> Hủy</button></td>
		</tr>';
	}
}
